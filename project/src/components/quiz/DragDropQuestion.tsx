...
const DropZone = ({ moveWord, words }) => {
  ...
  return (
   <div ref={drop} className="px-4 py-2 border rounded-md min-w-[300px] min-h-[100px] text-center flex flex-wrap gap-2 items-center justify-center">
...
const DragDropQuestion = ({ question, onAnswer }) => {
  ...
  const moveWord = (fromIndex) => {
    if (!userAnswer.includes(question.options[fromIndex])) {
      setUserAnswer([...userAnswer, question.options[fromIndex]]);
    }
  };
