...
const PythonFlashCards: React.FC<PythonFlashCardsProps> = ({ onComplete }) => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [isFlipped, setIsFlipped] = useState(false);

  const handleNext = () => {
    if (currentIndex < pythonFlashCards.length - 1) {
  ...
  return (
    <div className="max-w-2xl mx-auto p-8">
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-2xl font-bold">Python Fundamentals</h2>
        <div className="text-sm text-gray-400">
          {currentIndex + 1} / {pythonFlashCards.length}
        </div>
      </div>
          ...
          <div className={`absolute inset-0 backface-hidden ${isFlipped ? 'opacity-0' : 'opacity-100'}`}>
            <div className="h-full backdrop-blur-xl bg-white/10 rounded-2xl p-8 border border-white/20">
              <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <p className="text-xl">{pythonFlashCards[currentIndex].front}</p>
            ...
            <div className="h-full backdrop-blur-xl bg-white/10 rounded-2xl p-8 border border-white/20">
              <div className="flex flex-col justify-center h-full text-center">
                ...
                <p className="text-lg leading-relaxed whitespace-pre-line">
                  {pythonFlashCards[currentIndex].back}
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="flex justify-center">
        ...
        >
          {currentIndex === pythonFlashCards.length - 1 ? 'Start Quiz' : 'Next Card'}
