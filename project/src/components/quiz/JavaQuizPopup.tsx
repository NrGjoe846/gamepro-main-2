...
import JavaFlashCards from './flashcards/JavaFlashCards';

// Import Java questions data
...
const JavaQuizPopup: React.FC<JavaQuizPopupProps> = ({ isOpen, onClose, onComplete }) => {
  ...
  const findAllQuestions = () => {
    ...
    javaQuestionsData.forEach(phaseData => {
    ...
    return (
      ...
      >
        <div className="bg-[#1a1a2e] rounded-2xl p-6 max-w-4xl w-full m-4">
          <JavaFlashCards onComplete={handleFlashCardsComplete} />
        </div>
      </motion.div>
    );
  }
      ...
      <DndProvider backend={HTML5Backend}>
        ...
        <motion.div className="relative w-full max-w-4xl bg-[#1a1a2e] rounded-2xl shadow-2xl p-6 m-4 max-h-[90vh] overflow-y-auto">
          ...
          {!showResults ? (
            <>
              <div className="mb-6">
                <h2 className="text-xl font-bold">Java Quiz</h2>
